...
contract LendingData is ERC721Holder, ERC1155Holder, Ownable, ReentrancyGuard {
  ...
  address public nftAddress = 0xD7ACd2a9FD159E69Bb102A1ca21C9a3e3A5F771B;
  ...
  function payLoan(uint256 loanId) external payable {
    ...
    if ( discount != 1 ){
        ...
        if ( loans[loanId].currency == address(0) )
            require(msg.sender.send(interestToStaterPerInstallement.div(discount)),"ETH returnation failed");
    }
    
    uint256 amountPaidAsInstallmentToLender = interestPerInstallement.sub(interestToStaterPerInstallement).add(paidByBorrower.sub(interestPerInstallement)); // >> amount of installment that goes to lender
